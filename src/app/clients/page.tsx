...
import { getAllClients, useGetAllClientsQuery } from "@/api/clientSlice";
import { CLIENT_TH } from "@/config/data/live";
import { roboto_400, roboto_500 } from "@/config/fonts";
import { IClientsData, IClientsResponse } from "@/types/api/clients.types";
...
const TABS = ['Clients', 'Deposits', 'Withdrawals', 'Support tickets']
    ...
    <div className='h-[calc(100%-58px)] min-h-fit relative'>
      ...
      <div className="mt-12 flex flex-row w-fit h-[43px]">
        {TABS.map((x, i) => {
          ...
          return (
            <div key={i} onClick={() => setTab(x.toLowerCase())} className={`${roboto_500.className} text-[17px] hover:text-white hover:text-[18.5px] hover:h-[47.5px] transition-all duration-300 ${active ? 'text-white' : 'text-grey_800'} ${x.toLowerCase() == 'withdrawals' ? 'w-[110px]' : x.toLowerCase() == 'support tickets' ? 'w-[180px]' : 'w-[96px]'} text-center py-2.5 cursor-pointer h-[46px] ${active ? 'bg-[#0096D6C9]' : 'bg-black3'}`}>
              {x}
            </div>
          )
        })}
      ...
      <div className="bg-black3 min-h-[100%] flex-1 pt-12 px-5 md:px-10 lg:px-14 relative">
        ...
        <div className="min-h-[500px] relative w-full md:h-[80%] h-[100%] pb-10 mt-8 pr-5">
          <div className="absolute w-full py-5 pb-6 pl-0 -ml-4 sm:ml-0 sm:pl-3 pr-10 overflow-x-auto">
            <table className={`${roboto_400.className} w-[calc(100%-20px)] min-w-[810px] lg:ml-5`}>
              ...
              <tbody>
                {clientFilteredList.map((tx, indx) => {
                  return (
        ...
        <div className="mt-auto bg-black3 absolute bottom-10 z-50 flex flex-row items-center">
          <div
